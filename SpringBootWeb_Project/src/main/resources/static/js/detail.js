...
function test() {
    $.get("http://localhost:8080/queryDetail",{id:(Number)(sessionStorage.goodsId)},function (data) {
        console.log("6++++++"+data.goodsId);
        $("#mySwipe").append(
            "<div class='swipe-wrap'>"+
            "<div>" +
            "<img src='"+data.fileId+"' height='180px' width='100%'/>" +
            "</div>"+
            "</div>"
        );
        $(".main").append(
            "<div class='jj_title'>"+data.goodsName+"</div>" +
            "     <ul class='jj_list' style='margin-bottom:48px;'>" +
            "        <li data-id='"+data.goodsId+"'>" +
            "           <ul class='jj_info'>" +
            ...
            "              <li><span class='label'>商品数量：</span><span>"+data.goodsCount+"</span></li>" +
            "           </ul>\n" +
            "        </li>\n" +
            "     </ul>"
        );
