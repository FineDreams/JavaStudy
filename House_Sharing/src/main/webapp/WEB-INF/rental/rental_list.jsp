        ...
        <div id="main">
            <form action="${pageContext.request.contextPath}/user/queryRentals" method="post">
                ...
                <div class="search_add">                        
                    <div>地 点：<input type="text" name="place" value="${pageBean.place}" class="width100 text_search" /></div>
                    <div>户型：<input type="text" name="type" value="${pageBean.type}" class="width100 text_search" /></div>
                ...
                <div id="data">            
                    ...
                    <tr>
                            ...
                            <td class="td_modi">
                                <c:if test="${rental.uid==sessionScope.user.uid}">
                                    <input type="button" value="修改" class="btn_modify" onclick="location.href='${pageContext.request.contextPath}/user/toRentalModify?rid=${rental.rid}';" />
                                </c:if>
                                <c:if test="${rental.uid!=sessionScope.user.uid}">
                                    <c:if test="${rental.state=='2'}">
                                        <input type="button" value="租房" class="btn_start" onclick="setState(${rental.rid});" />
                                    ...
                                    <c:if test="${rental.state=='1'}">
                                        <input type="button" value="已出租" class="btn_pause"/>
                                        <input type="button" value="查看" class="btn_modify" onclick="location.href='${pageContext.request.contextPath}/user/toRentalDetail?rid=${rental.rid}';" />
                                    </c:if>
                ...
                <!--分页-->
                <div id="pages">
                    <a href="${pageContext.request.contextPath}/user/queryRentals?nowPageIndex=${pageBean.startPageIndex}">首页</a>
        	        <c:if test="${pageBean.nowPageIndex>1}">
                        <a href="${pageContext.request.contextPath}/user/queryRentals?nowPageIndex=${pageBean.nowPageIndex-1}">上一页</a>
                    </c:if>
                    ...
                    <c:forEach begin="${pageBean.startPageIndex}" end="${pageBean.endPageIndex}" var="page">
                        ...
                        <c:if test="${pageBean.nowPageIndex!=page}">
                            <a href="${pageContext.request.contextPath}/user/queryRentals?nowPageIndex=${page}">${page}</a>
                        </c:if>
                    </c:forEach>

                    <c:if test="${pageBean.nowPageIndex<pageBean.pageCount}">
                        <a href="${pageContext.request.contextPath}/user/queryRentals?nowPageIndex=${pageBean.nowPageIndex+1}">下一页</a>
                    </c:if>
                    <a href="${pageContext.request.contextPath}/user/queryRentals?nowPageIndex=${pageBean.pageCount}">末页</a>
                </div>                    
            </form>
        </div>
        <!--主要区域结束-->
