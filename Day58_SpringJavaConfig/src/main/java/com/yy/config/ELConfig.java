...
@PropertySource("classpath:com/yy/show.properties")//指定资源文件的路径
public class ELConfig {
    ...
    //4、注入其他bean的属性
    @Value("AopDemoService")
    private String fromAnother;

    //5.注入文件资源
    @Value("classpath:com/yy/show.txt")
    ...
    //8、若要使用value织入，则需要搭配如下静态方法
    public static PropertySourcesPlaceholderConfigurer placeholderConfigurer(){
        return new PropertySourcesPlaceholderConfigurer();
    }
    //9.注入资源文件后，也可以在environment对象中获得资源文件中的内容
    @Autowired
    ...
    public void outputResource(){
        try {
            ...
            System.out.println("environment.getProperty('author'):---"+environment);
            System.out.println();
        }catch (IOException e){
            e.printStackTrace();
        }
